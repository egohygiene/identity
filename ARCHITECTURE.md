...
Identity uses a layered, contract-driven architecture. This document owns structural boundaries, dependency direction, integration rules, and current-to-target evolution. Logical responsibilities remain canonical in [SYSTEM.md](SYSTEM.md).
...
flowchart LR
  S1[Identity schema]
  S2[Creative brief builder]
  S3[Asset planner]
  S4[Raster and vector adapters]
  S5[Text and metadata generator]
  S6[Validation and manifest]
  S7[Consumer integration]
  S1 --> S2
  S2 --> S3
  S3 --> S4
  S4 --> S5
